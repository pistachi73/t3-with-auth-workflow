import type { Icon } from "./icons.type";

export const GoogleIcon: Icon = ({ className }) => {
  return (
    <svg
      className={className}
      width="40"
      height="40"
      viewBox="0 0 40 40"
      fill="currentColor"
      xmlns="http://www.w3.org/2000/svg"
    >
      <title>Google Icon</title>
      <path
        d="M39.611 16.083H38V16H20V24H31.303C29.654 28.657 25.223 32 20 32C13.373 32 8 26.627 8 20C8 13.373 13.373 8 20 8C23.059 8 25.842 9.154 27.961 11.039L33.618 5.382C30.046 2.053 25.268 0 20 0C8.955 0 0 8.955 0 20C0 31.045 8.955 40 20 40C31.045 40 40 31.045 40 20C40 18.659 39.862 17.35 39.611 16.083Z"
        fill="#FBC02D"
      />
      <path
        d="M2.30664 10.691L8.87764 15.51C10.6556 11.108 14.9616 8 20.0006 8C23.0596 8 25.8426 9.154 27.9616 11.039L33.6186 5.382C30.0466 2.053 25.2686 0 20.0006 0C12.3186 0 5.65664 4.337 2.30664 10.691Z"
        fill="#E53935"
      />
      <path
        d="M20.0003 40C25.1663 40 29.8603 38.023 33.4093 34.808L27.2193 29.57C25.2113 31.091 22.7153 32 20.0003 32C14.7983 32 10.3813 28.683 8.71731 24.054L2.19531 29.079C5.50531 35.556 12.2273 40 20.0003 40Z"
        fill="#4CAF50"
      />
      <path
        d="M39.611 16.083L39.595 16H38H20V24H31.303C30.511 26.237 29.072 28.166 27.216 29.571L27.219 29.569L33.409 34.807C32.971 35.205 40 30 40 20C40 18.659 39.862 17.35 39.611 16.083Z"
        fill="#1565C0"
      />
    </svg>
  );
};
